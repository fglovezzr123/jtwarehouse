...
import com.tojoy.cn.emay.eucp.inter.http.v1.dto.response.ResponseData;
import com.tojoy.cn.emay.util.Md5;
import com.tojoy.cn.emay.util.http.EmayHttpClient;
import com.tojoy.cn.emay.util.http.EmayHttpRequestKV;
import com.tojoy.cn.emay.util.http.EmayHttpResponseStringPraser;
import com.tojoy.cn.emay.util.http.EmayHttpResultCode;
...
import com.tojoy.cn.emay.eucp.inter.http.v1.dto.response.SmsResponse;
import com.tojoy.cn.emay.util.DateUtil;
import com.tojoy.cn.emay.util.JsonHelper;
import com.tojoy.cn.emay.util.http.EmayHttpResponseString;
...
//	public static final String SMSPRE =ApplicationPath.getParameter("SMSPRE") ;// 签名5
	// public static final String SMSPRE = "天九集团"; // 签名2
	public static final String SMSPRE_APP ="企服云" ; // 签名3
//	public static final String SMSPRE_APP =ApplicationPath.getParameter("SMSPRE_APP") ; // 签名3
	...
	public static class MsmTemplateId {
		...
		/**
		 * 活动结束，template:${name}老板，您在平台发布的${hdname}活动已结束，报名费共计：${fy}元，款项已到账，赶紧确认下吧。
		 */
		...
		/**
		 * 会议开始提醒，template:${name}老板，您所报名的${hyname}会议于${time}正式开始啦，请您提前持入场凭证参加会议，并将手机调至震动模式。收看直播请立即前往。
		 */
		...
		/**
		 * 商洽及应答被采纳，template:${name}老板，您在${subname}所发布的${type}您共计获得${money}J币，已经放入您的账户，还请尽快登录查看。
		 */
		...
		/**
		 * 直播开始前通知，template:${name}老板，您所支付的${livename}直播于${starttime}正式开始，还有3分钟就开始了，精彩不容错过。

		 */
		public static final String LIVE_START = "SMS_90065045";
		/**
		 * 直播报名，template:${name}老板，看这里！您于${paytime}成功支付${livename}直播。直播开始时间为：${playtime}，届时记得收看！
		 */
	...
	private static boolean setSms(String appId, String sign, String timestamp, String host, String content, String mobiles,
	...
	public static boolean sendSms1(String templateId, String recNum, String param) {
		try {
			TaobaoClient client = new DefaultTaobaoClient("http://gw.api.taobao.com/router/rest", "23736541", "a9990790515e900198fc3111b10dd34a");
...
//		System.out.println("sms_send_result:" + URL+" "+APP_KEY+" "+APP_SECRET);
	...
	public static void main(String[] args) {
		// boolean bo = sendSms(AldyMessageUtil.MsmTemplateId.REG,
		// "13760262786", "{code:'123421',product:'天九云'}");
		// boolean bo = sendSms(AldyMessageUtil.MsmTemplateId.RECHARGE,
		// "13760262786", "{name:\"张三\",time:\""
		// + DateUtils.getCurrTimeStr("yyyy-MM-dd HH:mm") +
		// "\",money:\"RMB666.00元\",ye:\"666.00元\"}");
		//boolean bo = sendSms(AldyMessageUtil.MsmTemplateId.NEGOTIATE_SUCCESS, "15670665119", "{name:\"真水无香\",subname:\"天九共享网\",type:\"合作信息已被采纳，棒棒哒！\",money:\"4444\"}");
		//boolean bo2 = sendSms(AldyMessageUtil.MsmTemplateId.NEGOTIATE_SUCCESS, "15670665119", "{name:\"真水无香\",subname:\"天九共享网诸葛解惑内\",type:\"应答已被采纳，好厉害！\",money:\"5555\"}");
//		boolean bo = sendSms("SMS_79100083", "13760262786",
//				"{name:\"张三\",time:\"2016-03-01 13:21\",money:\"546元\",czType:\"互助宝\",ye:\"687元。互助宝，帮您在刀刃上花钱\"}");
//		System.out.println(bo);
		// String ss =
		// "{\"alibaba_aliqin_fc_sms_num_send_response\":{\"result\":{\"err_code\":\"0\",\"model\":\"102117321861^1102751793533\",\"success\":true},\"request_id\":\"3gsj121wqtua\"}}";
		// System.out.println(parseResult(ss));
	...
	public static String activityover(String username, String title, Double fy) {
		String content = "【" + SMSPRE + "】" + username + "老板，您在平台发布的" + title + "活动已结束，报名费共计：" + fy + "元，款项已到账，赶紧确认下吧。";
		return content;
	}
	...
	public static String activityrefund(String username, String title, Double fy) {
		String content = "【" + SMSPRE + "】" + username + "老板，您在平台报名的" + title + "活动已退款，退款金额：" + fy + "元，款项已到账，赶紧确认下吧。";
		return content;
	}
	 ...
	 * @param starttime
	 * @param id
	 * 【天九共享会】**老板，您所报名的****活动已延期至2017-09-08 19:00:00，还请您尽快与活动主办人核实信息。立即前往
	 * @return
	 */
	public static String activitydelay(String username, String title, String starttime, String id) {
		String content = "【" + SMSPRE + "】" + username + "老板,您所报名的" + title + "活动已延期至" + starttime
				+ "，还请您尽快与活动主办人核实信息。<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/activity/activityDetailPage.do?id=" + id
				+ "'>立即前往</a>";
//		String content = "【" + SMSPRE + "】尊敬的" + username + ",您所报名的" + title + "活动已延期至" + starttime
//				+ "，望尽快与活动主办人核实。<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/activity/activityDetailPage.do?id=" + id
//				+ "'>立即前往</a>";
		return content;
	}
	 ...
	 * @param titles
	 * @param mobile
	 * 【天九共享网】**老板，恭喜恭喜，您于2016-10-32 19:00成功报名****会议，出席会议别忘了携带会议凭证。凭证信息为：姓名：***，电话：********。
	 * @return
	 */
	public static String meetingsignup(String nickname, String signupdate, String titles, String mobile) {
//		String message = "【" + SMSPRE + "】尊敬的" + nickname + "，" + "您于" + signupdate + "成功报名" + titles + "会议，会议凭证为：姓名："
//				+ nickname + "，电话：" + mobile + "。";
		String message = "【" + SMSPRE + "】" + nickname + "老板，恭喜恭喜，您于" + signupdate + "成功报名" + titles + "会议，出席会议别忘了携带会议凭证。凭证信息为：姓名："
				+ nickname + "，电话：" + mobile + "。";
		return message;
	}
	 ...
	 * @param id
	 * @return
	 * 【天九共享会】**老板，您所关注的****会议于2016-03-01开始报名啦。千万不要错过！立即前往
	 */
	public static String meetingysignupremind(String nickname, String signupdate, String titles, String id) {
		String message = "【" + SMSPRE + "】" + nickname + "老板，您所关注的" + titles + "会议于" + signupdate
				+ "开始报名啦。<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/meeting/detail/index.do?id=" + id + "'>立即前往</a>";
//		String message = "【" + SMSPRE + "】尊敬的" + nickname + "，" + "您所收藏的" + titles + "会议于" + signupdate
//				+ "开始报名。<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/meeting/detail/index.do?id=" + id + "'>立即前往</a>";
		return message;
	}
	 ...
	 * @param titles
	 * @param id
	 * 【天九共享会】【天九共享网】**老板，您所报名的****会议于 2016-03-01 19:00正式开始啦，请您提前持入场凭证参加会议，并将手机调至震动模式。收看直播请立即前往。
	 * @return
	 */
	public static String meetingystartremind(String nickname, String startdate, String titles, String id) {
		String message = "【" + SMSPRE + "】" + nickname + "老板 ，您所报名的" + titles + "会议于" + startdate + "正式开始啦，"
				+ "请您提前持入场凭证参加会议，并将手机调至震动模式。收看直播请<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/meeting/detail/index.do?id="
				+ id + "'>立即前往</a>。";
//		String message = "【" + SMSPRE + "】尊敬的" + nickname + "，您所报名的" + titles + "会议于" + startdate + "正式开始，"
//				+ "请您提前持入场凭证参加会议，并将手机调至震动模式。<a href='" + ConfigUtil.DOMAIN + "/wxfront/m/meeting/detail/index.do?id="
//				+ id + "'>立即前往</a>";
		return message;
	}
	 ...
	 * @param name
	 * @return
	 * 【天九共享会】恭喜恭喜，**老板，您提交的认证信息已通过审核啰，再次恭喜您成为天九共享会认证用户。天九在手，老板无忧。
	 */
	 ...
	 * @param name
	 * @return
	 * 【天九共享会】**老板，非常抱歉，您提交的认证信息未通过审核，还请继续完善认证信息，确保认证成功哦，平台优质服务等待您。
	 */
	public static String userreconrefuse(String name) {
		String content = "【" + SMSPRE + "】" + name + "老板，非常抱歉，您提交的认证信息未通过审核，还请继续完善认证信息，确保认证成功哦，" + SMSPRE + "各项优质服务等待您。如有问题请拨打客服电话010-53118922。";
		return content;
	}
	 ...
	 * @param name
	 * @return
	 * 【天九共享会】终于等到你，**老板，热烈欢迎您登录天九共享网，来到这里说明您已注册，越早进行认证，越早收获优质服务哦^^。如遇操作问题，请联系专属服务秘书。
	 */
	public static String userreconremind(String name) {
		String content = "【" + SMSPRE + "】终于等到你，" + name + "老板，热烈欢迎您登录天九共享网，来到这里说明您已注册，越早进行认证，越早收获优质服务哦^^。如遇操作问题，请联系专属服务秘书。";
//		String content = "【" + SMSPRE + "】尊敬的" + name + "，欢迎登录天九云平台，您已成功注册。请您尽快进行认证，收获优质服务。";
		return content;
	}
	...
	public static String userwallettxsuccess(String name, String createtime, Double txje, Double syje) {
		String content = "【" + SMSPRE + "】" + name + "老板，您的账户于" + createtime + "成功提现RMB" + txje + "元，您当前账户余额为RMB" + syje
				+ "元。请确认好资金安全。";
		return content;
	}
	...
	public static String userwallettxfail(String name, String createtime, Double txje, Double ye) {
		String content = "【" + SMSPRE + "】" + name + "老板，您的账户于" + createtime + "申请的提现RMB" + txje
				+ "元审核失败，现已归还到您的账户余额，当前您的账户余额为RMB" + ye + "元。请确认好资金安全。";
		return content;
	}
...
//				+ "老板，热烈欢迎您登录天九共享网，来到这里说明您已注册，越早进行认证，越早收获优质服务哦^^。如遇操作问题，请联系专属服务秘书。";
		String content = "【" + SMSPRE + "】尊敬的" + name + "，恭喜您已成为天九共享会尊贵的会员！点击此链接查看：天九共享会功能介绍及用户公约(<a style='color:blue;' href='http://t.cn/R9tLmMS'>http://t.cn/R9tLmMS</a>)。";
		return content;
	}
	 ...
	 * @param integer
	 * @return
	 * 【天九共享网】**老板，您在*****所发布的合作信息已被采纳，棒棒哒！您共计获得共计***J币，已经放入您的账户，还请尽快登录查看，立即前往。
	 */
	public static String userRewardAccept(String name, String question, Integer integer) {
		String content = "【" + SMSPRE + "】" + name + "老板,您在" + question + "所发布的合作信息已被采纳，棒棒哒！您共计获得共计" + integer + "J币，已经放入您的账户，还请尽快登录查看，立即前往。";
		return content;
	}
	...
	public static String userzgRewardAccept(String name, String question, Integer integer) {
		String content = "【" + SMSPRE + "】" + name + "老板,您在" + question + "诸葛解惑内所发布的应答已被采纳，好厉害！您共计获得共计" + integer + "J币，已经放入您的账户，还请尽快登录查看。";
		return content;
	}
	...
	public static String receiveCoupon(String name, Integer integer,String cointype) {
		String content = "【" + SMSPRE + "】" + name + "老板,恭喜您获得" + integer + cointype +"兑换券，已兑换成功，" + integer + cointype +"已存入钱包，请注意查收。";
		return content;
	}
}
