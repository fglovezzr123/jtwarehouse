...
--%>
	...
	<form  action="quickDoor/update.do"  id="quickDoor_form"  method="post"   onsubmit="return validateSubmitForm(this)">
		 <input id="quickImgPath" name="quickImgPath" value="${dto.quickImgPath }"  type="hidden" />
		<div class="divider"></div>
		  <table class="table table-nobordered " style="margin-top: 25px;">
			  <tr>
			    	<th style="width: 120px">入口名称：</th>
			    	<td>
			     		<input type="text" name="quickName" value="${dto.quickName }" class="easyui-validatebox" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
		 
		 	  <tr>
			    	<th style="width: 120px">入口图片：</th>
			    	<td >
                		<span id="updateQuickDoorImgs" title="点击删除" ></span>
	                    ...
	                    <iframe id="uploadPicFrame" src="" style="display:none;"></iframe>  
						 <strong style="color:#F00">尺寸100*100px。</strong>
                	</td>
			  </tr>
			 <!-- <tr>
			    	<th style="width: 80px">所属模块：</th>
			    	<td>
			     		<select name="quickModule" >
						<option  value="0">首页</option>
					    </select>
			    	</td>
			  </tr> -->
			  <tr>
			    	<th style="width: 80px">入口序号：</th>
			    	<td>
			     		<input type="text" name="quickSortNum"  onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"  value="${dto.quickSortNum }" class="easyui-numberbox" required="true" min="1" max="9999"/>
			    	</td>
			  </tr>
			  <tr>
				 <th></th>
		...
		<input type="hidden" name="quickStatus" value="${dto.quickStatus}"/>
	</form>
...
function save(){
	...
	if($.trim(picUrl).length <= 0){
		Msg.alert("提示","请上传快捷入口图片！","warning",null);
		return false;
	}
	$("#quickImgPath").val(picUrl);
	if(flag){
		if(!validateSubmitForm($('#quickDoor_form'))){
			return;
		}
	}
}

var picUrl = $("#quickImgPath").val();
if($.trim(picUrl).length > 0){
	$("#updateQuickDoorImgs").html("");
	...
	$("#updateQuickDoorImgs").append(imgHtml);
}
...
function initUploadTagPicFrame(){
	...
	frameParam.picRepository = "qfyEntryQuickDoor";
