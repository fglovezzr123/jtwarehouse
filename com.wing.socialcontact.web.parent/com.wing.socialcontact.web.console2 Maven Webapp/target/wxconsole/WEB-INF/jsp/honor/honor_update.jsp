...
<div style="width: 800px;margin: 0 auto;" >
	<form  action="honor/update.do"   method="post" id="news_form"  afterCallback="saveCallback" enctype="multipart/form-data" >
		  <input id="picPath" name="picPath" value="${b.picPath}"  type="hidden" />
		  <input id="picPath2" name="picPath2" value="${b.picPath2}"  type="hidden" />
		 <table class="table table-nobordered " style="margin-top: 25px;">
			  
			  <tr>
			    	<th style="width: 120px">点亮勋章图标：</th>
			    	<td >
	                    ...
	                    <strong style="color:#F00">请上传jpg,jpeg,png格式图片,建议尺寸200*200px，大小限制200k。</strong>          	
                	</td>
			  </tr>
			  <tr>
			    	<th style="width: 120px">非点亮勋章图标：</th>
			    	<td >
                		<span id="nimgs2" title="点击删除" ></span>
                	   	<input  onclick="doSelectPic2()"  type="button" value="上传图片" id="picturefile" class="form_shangchuan" />
	                    <iframe id="uploadPicFrame2" src="" style="display:none;"></iframe>
	                    <strong style="color:#F00">请上传jpg,jpeg,png格式图片,建议尺寸200*200px，大小限制200k。</strong>          	
                	</td>
			  </tr>
			  <tr>
			    	<th style="width: 120px">勋章类型：</th>
			    	<td >
			     	    <select class="combox required" class="easyui-validatebox" required="true" id="honorType" name="hornorType" sValue="${b.hornorType}">
			  ...
			  <tr>
			    	<th style="width: 120px">勋章编码：</th>
			    	<td >
			     		<input type="text" name="honorCode" class="easyui-validatebox" required="true"   value="${b.honorCode}" />
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 120px">勋章名称：</th>
			    	<td >
			  ...
			  <tr>
			   ...
			   <tr>
			    	<th style="width: 120px">勋章详情：</th>
			    	<td>
					<textarea id="remark" name="remark" style="width:320px;height:600px;" >${b.remark}</textarea>
			    	</td>
			  </tr>
			  <tr>
				 <th></th>
...
$(document.body).ready(function(){
	editor = KindEditor.create('#remark',options);
})
function save(){
	var flag = true;
	var picUrl = "";
	$("#nimgs").find('.up_pic_img').each(function(){
	...
	var picUrl2 = "";
	$("#nimgs2").find('.up_pic_img').each(function(){
		picUrl2 = $(this).attr("srcpath");
	});
	if($.trim(picUrl2).length <= 0){
		alert("请上传非点亮勋章图标");
		return false;
	}
	$("#picPath2").val(picUrl2);
	if(flag){
...
if($.trim(picUrl2).length > 0){
	$("#nimgs2").html("");
	var imgHtml = "<img title='点击删除' onclick='delPic(this)' srcpath='"+picUrl2+"' src='"+oss+picUrl2+"' class='up_pic_img' style='width:240px; height:160px;'/>";
	$("#nimgs2").append(imgHtml);
}
...
function doSelectPic2() {
	...
	if(imgSize <= 0 ){
		$("#pic", $("#uploadPicFrame2")[0].contentWindow.document).click();
	}else{
		alert("仅能上传一张图片");
	}
	return false;
...
initUploadPicFrame2();
function initUploadPicFrame2(){
	var frameSrc = "<%=basePath%>news/getUploadPicForm.do";
	var frameParam = new Object();
	frameParam.formId= "upload";
	frameParam.moduleName= "usercenter";
	frameParam.inputId= "pic";
	frameParam.inputOnChange = "parent.picChange2";
	frameParam.jsonp = "parent.picUploadCallback2";
	frameParam.ysStyle = "YS200200";
	frameSrc += "?"+parseParam(frameParam);
	$("#uploadPicFrame2").attr("src", frameSrc);
}
//公共方法,用来将对象转化为URL参数
...
function picChange2(inputFile){
    ...
    if (fileSize>2097152){
		...
		$("#upload", $("#uploadPicFrame2")[0].contentWindow.document).submit();
	}
}
...
//上传完成后回调的方法
function picUploadCallback2(data){
	if (data.returnCode == "1"){
		var picUrl = data.picPath;
		if(picUrl.length > 0){
			var imgHtml = "<img title='点击删除' onclick='delPic(this)' srcpath='"+data.picPath+"' src='"+data.img_url+"' class='up_pic_img' style='width:240px; height:160px;'/>";
