...
<script type="text/javascript">	
						...
						},
						{
							...
							formatter: function(rowIndex, rowData){
								...
								str += "<a href = 'javaScript:void(0)'  onclick = showviewc('"+rowData.id+"')>查看</a>&nbsp;&nbsp;"+
							     "<a href = 'javaScript:void(0)' onclick = deleteCou('"+rowData.id+"')>删除</a>&nbsp;&nbsp;" +
								"<a href='javaScript:void(0)'  onclick=plsc('"+rowData.id+"','"+rowData.couponType+"')>批量生成</a>";
								if (rowData.couponType==4) {
								    if (rowData.state==0) {
                                        str += "&nbsp;&nbsp;<a href='javaScript:void(0)' onclick=changeState('"+rowData.id+"')>设为有效</a>";
                                    } else if (rowData.state==1) {
                                        str += "&nbsp;&nbsp;<a href='javaScript:void(0)' onclick=changeState('"+rowData.id+"')>设为无效</a>";
                                    }
                                }
								return  str;
							}
	...
	function changeState(id) {
        Msg.confirm('提示',"确定要改变吗？<br/>", function(r){
            if (r){
                var pm = {"id":id};
                var dg = $('#<%=request.getParameter("rel")%>_datagrid');
                selectedTodo_doPost("coupon/changeState.do",pm,dg,'');
            }
        });
    }
	...
	//批量生成
	function plsc(fid,type){
		MUI.openDialog('批量生成','coupon/batchGeneratePage.do?id='+fid+'&type='+type+'&rel=<%=request.getParameter("rel")%>','<%=request.getParameter("rel")%>_plsc',{width:450,height:300});
	}
	//删除
