...
<%@ include file="/WEB-INF/jsp/commons/include.inc.jsp"%>
					...
					<span style="float: left;"> 
						<%--<shiro:hasPermission name="entryQuickDetailBanner:add">--%>
							<a class="easyui-linkbutton" icon="icon-add" plain="true"
								href="financeProduct/addPage.do?rel=${param.rel }" title="新增" target="dialog"
								width="600" height="550" rel="${param.rel}_add"><span>新增</span>
							</a>
						<%--</shiro:hasPermission> --%>
						<%--<shiro:hasPermission name="entryQuickDetailBanner:update">--%>
							<a class="easyui-linkbutton" icon="icon-edit" plain="true" title="修改"
								href="financeProduct/updatePage.do?id={id}&rel=${param.rel}"
								target="dialog" width="600" height="550"
								rel="${param.rel}_update" warn="请先选择一条记录"><span>修改</span>
							</a>
						<%--</shiro:hasPermission>--%>
						<%--<shiro:hasPermission name="entryQuickDetailBanner:delete">--%>
					   		<a  class="easyui-linkbutton"  icon="icon-remove"	plain="true"
							href="financeProduct/del.do" target="selectedTodo"  title="确定要删除吗?" warn="请至少选择一条记录">批量删除</a>
				       <%--</shiro:hasPermission>--%>
						</span> 
		...
		<table id="financeProduct_datagrid" toolbar="#${param.rel}_toolbar"></table>
...
<script type="text/javascript">	
	...
	function loadMain(){
        alert("1111");
		$('#financeProduct_datagrid').datagrid(
			{
				url : "financeProduct/query.do",
				columns : [ [
						{
							field:"ck",
							title : "勾选",
