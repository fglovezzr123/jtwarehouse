...
--%>
	...
	<form  action="dept/add.do"   method="post"  id="dept_form"  onsubmit="return validateSubmitForm(this)">
			...
			<tr>
				...
				<td>
					<a  href="user/lookUpPage.do?type=1"  lookupGroup="user" title="用户查询">
						<input type="text"  readonly="readonly"  id="username"  toName="user.trueName"  class="easyui-validatebox" />
					</a>
					...
					<a href="dept/lookUpPage.do?type=3" lookupGroup="dept"  title="上级部门查询">
						...
						<c:choose>
							<c:when test="${empty param.superId }">
								<input  type="text" value="顶级部门"  id="username" readonly="readonly"  toName="dept.deptName"  class="easyui-validatebox" required="true"/>
							</c:when>
							<c:otherwise>
								<input  type="text"   id="username" value=" <my:outDeptName id="${param.superId }" /> "  readonly="readonly"  toName="dept.deptName"  class="easyui-validatebox" required="true"/>
							</c:otherwise>
						</c:choose>
