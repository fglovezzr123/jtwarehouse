...
<div style="width: 98%;margin: 0 auto;" >
	<form  action="pageConfig/pageColumn/update.do"  method="post" id="pageColumn_form" enctype="multipart/form-data" >
		<h3>栏目基础信息</h3>
		...
		<div id="pageElement">
			<c:forEach items="${b.elementList }" var="element">
				<table class="table table-bordered">
					...
					<tr>
						<th width="10%">标题1:</th>
						<td width="40%"><input type="text" name="titleOne" class="easyui-validatebox" required="true" data-options="validType:['length[1,20]']"  maxlength="20" value="${element.titleOne }"/></td>
					...
					<tr>
						...
						<td>
							<a href="" lookupGroup="content" title="元素内容链接查询" width="1000" height="500" targetBox="td" warn="请先选择显示内容">
								<input type="text" name="contentName" toName="content.name" class="easyui-validatebox" required="true" readonly="readonly" value="${element.contentName }"/>
								<input type="hidden" name="contentId" toName="content.id" value="${element.contentId }"/>
							</a>
							<input type="text" name="detailUrl" value="${element.detailUrl }" class="easyui-validatebox" required="true" style="display: none;width: 280px;"/>
		...
		<input type="hidden" name="columnType" value="${b.columnType }"/>
