...
<div style="width: 800px;margin: 0 auto;" >
	<form  action="club/add.do"  method="post" id="club_form" afterCallback="saveCallback" enctype="multipart/form-data" >
			...
			<tr>
					...
					<td>
						<a href="clubclass/lookUpPage.do?type=1"  lookupGroup="clubclass" title="分类查询">
							<input type="text"  readonly="readonly" toName="clubclass.name"  class="easyui-validatebox" required="true" value='<my:outClubClassName id="${param.classId }"/>'/>
						</a>
						<input type="hidden" name="classId"  toName="clubclass.id"  value="${param.classId }"/>
					</td>
			</tr>
			  <tr>
			    	<th style="width: 80px">会所地址：</th>
			    	<td >
			     		<input type="text" name="address" class="easyui-validatebox" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">会所电话：</th>
			    	<td >
			     		<input type="text" name="phone" class="easyui-validatebox"  required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
...
var options = {
        ...
        filterMode : false,
		uploadJson:'news/upload.do?ysStyle=YS640386&moduleName=club',
		width : '320px',
		height:'600px',
		resizeType : 1,
