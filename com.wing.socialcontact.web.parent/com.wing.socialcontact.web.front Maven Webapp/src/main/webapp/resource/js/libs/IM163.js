...
function commons(){
  ...
  if(window.location.href.indexOf('talkP2P')>-1 || window.location.href.indexOf('talkAbout')>-1 || window.location.href.indexOf('talkAboutQl')>-1 ){
	  console.log("公共IM不再执行");
	  return;
  }
  ...
  var mytoken='';
  var mycount=localStorage.getItem("currentUserIdfFromIm")
  zdy_ajax({
...
function onSessions(sessions) {
    console.log('收到会话列表', sessions);
     for(var i=0;i<sessions.length;i++){
    	 saveLocalSession(sessions[i].to,sessions[i],'');
    }
   // saveLocalSession(sessions.to,sessions,'');
    data.sessions = nim.mergeSessions(data.sessions, sessions);
    updateSessionsUI();
}
function onUpdateSession(session) {
    console.log('会话更新了', session);
    saveLocalSession(session.to,session);
...
commons();
function saveLocalSession(pushID,session,cur){
	var datas;
    var myfriendP='';
    var myfriendNicname='';
