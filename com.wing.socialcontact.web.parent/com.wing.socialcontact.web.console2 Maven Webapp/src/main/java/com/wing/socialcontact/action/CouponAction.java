...
public class CouponAction extends BaseAction {
    ...
    public ModelAndView add(Coupon coupon, Errors errors) {
        ...
        coupon.setCouponCoinType(coupon.getCouponType() == 4 ? null : coupon.getCouponCoinType());
        if (coupon.getCouponType() == 4 && coupon.getState() == 1) {
            //判断是否存在有效，不能多次添加有效
            Coupon searchModel = new Coupon();
            searchModel.setCouponType(coupon.getCouponType());
            searchModel.setState(coupon.getState());
            List<Coupon> coupons = couponService.selectCoupons(searchModel);
            if (!CollectionUtils.isEmpty(coupons)) {
                return ajaxDoneTextError("目前只能有一种有效的会议券！");
            }
        }
    ...
    public String batchGeneratePage(String id, int type, ModelMap map) {
    ...
    public ModelAndView batchGenerate(CouponGenerate cg, Errors errors, String startTime, String endTime)
        ...
        if (coupon.getUseRange().equals("1") || coupon.getUseRange().equals("4")) {
            ...
            if (coupon.getCouponType() == 4) {
                ...
                pageParam.setPage(1);
                pageParam.setRows(1);
                DataGrid data = couponGenerateService.selectDuplicate(pageParam, cg);
                if (data.getTotal() > 0) {
                    return ajaxDoneTextError("时间段被占用！");
                }
            }
            cg.setStartTime(sdf1.parse(startTime));
            cg.setEndTime(sdf1.parse(endTime));
    ...
    public String loadCL(String id, ModelMap map) throws ParseException {
        ...
        String isff = "1";
        if (cg.getRemainNum() == null || cg.getRemainNum() == 0) {
            isff = "0";
        }
        String isgq = "1";
    ...
    public ModelAndView grantCoupon(String mobeils, String fkId) {
        ...
        if (cg.getStartTime().after(now)) {
            return ajaxDoneError("未到时间领取");
        } else if (cg.getEndTime().before(now)) {
            return ajaxDoneError("已过期");
        }
        Coupon c = couponService.selectById(cg.getFkId());
        for (int i = 0; i < arry.length; i++) {
            receiveCoupon(arry[i], fkId);
        }
        if (c.getCouponType() != 4) {
     ...
     *
     * @param mobile 手机号
     * @param fkId   优惠券生成记录表id
     * @return
     */
    private void receiveCoupon(String mobile, String fkId) {
        if (StringUtil.isEmpty(mobile) || StringUtil.isEmpty(fkId)) {
            return;
        }
        ...
        cg.setReceiveNum(cg.getReceiveNum() + 1);
        couponGenerateService.updateCG(cg);
        return;
    }
    ...
    public ModelAndView changeState(String id) {
        Coupon coupon = couponService.selectById(id);
        if (coupon != null) {
            ...
            return ajaxDoneTextError("未查到实体");
        }
    }
