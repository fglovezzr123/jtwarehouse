...
--%>
	...
	<form method="post" action="dept/update.do"  id="dept_form"   onsubmit="return validateSubmitForm(this)">
		 ...
		 <div class="divider"></div>
			...
			<tr>
				...
				<td>
					<input type="text" name="deptSort" maxlength="3" class="easyui-numberbox"   required="true"  min="1" max="999" size="25" value="${requestScope.dept.deptSort}" />
			...
			<tr>
				<th>部门名称：</th>
				<td>
					<input type="text" name="deptName" class="easyui-validatebox" required="true" maxlength="50" size="25" value="${requestScope.dept.deptName}" />
				</td>
			</tr>
			<tr>
				<th>电话：</th>
			...
			<tr>
				...
				<td>
					...
					<input type="text"  readonly="readonly"  id="username"  toName="user.trueName"   value="<my:outTrueName id="${requestScope.dept.leadUid}"/>"/>
					</a>
				...
				<td>
					...
					<c:choose>
						<c:when test="${requestScope.dept.superId=='0'}">
							<input class="required" type="text" readonly="readonly" size="25" toName="dept.deptName" value="无上级" />
...
function save(){
	...
	if(flag){
		if(!validateSubmitForm($('#dept_form'))){
			return;
		}
	}
}
