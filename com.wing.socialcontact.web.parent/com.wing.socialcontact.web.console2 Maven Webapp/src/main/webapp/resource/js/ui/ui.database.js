	...
	$.extend({
		...
		bringBack: function(args){
			...
			$box.find("[toName^='"+_lookup["currentGroup"]+"']:input").each(function(){
				var $input = $(this), inputName = $input.attr("toName"), inputName2=$input.attr("name");
				if(inputName.substring(inputName.indexOf(".")+1) != 'no'){
					$input.setNewValue(args[inputName.substring(inputName.indexOf(".")+1)]);
					var callback=$input.attr("callback");
					if(undefined != callback && null != callback && callback.length >0 ){
						eval(""+callback+"('"+inputName2+"');");
					}
				}
