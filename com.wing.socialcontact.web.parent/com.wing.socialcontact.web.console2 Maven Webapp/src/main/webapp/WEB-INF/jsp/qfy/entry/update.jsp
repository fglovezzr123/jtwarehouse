...
%>
	...
	<form  action="qfy/entry/update.do"  id="entryPrise_update_form"  method="post" onsubmit="return validateSubmitForm(this)">
		...
		<div class="divider"></div>
		  <table class="table table-nobordered " style="margin-top: 10px;">
			 <tr>
		    	...
		    	<td>
		     		<input type="text" name="name" value="${dto.name }" class="easyui-validatebox" required="true"  data-options="validType:['length[1,100]']"   maxlength="100" />
		    	</td>
			  </tr>
			  <tr>
			  	<th style="width: 80px">联盟标题：</th>
		    	<td>
		     		<input type="text" name="title" value="${dto.title }" class="easyui-validatebox" required="true"  data-options="validType:['length[1,100]']"   maxlength="100" />
		    	</td>
			  </tr>
			 <tr>
		    	<th style="width: 80px">已服务数量：</th>
		    	<td>
		     		<input type="text" name="serviceCount"  value="${dto.serviceCount }" class="easyui-numberbox" required="true" min="1" max="99999999"/>&nbsp;&nbsp;家
		    	</td>
			  </tr>
			  <tr>
			  ...
			  <tr>
		    	...
		    	<td>
		     		<input type="text" name="ssUserId" id="ssUserId" value="${dto.ssUserId }" class="easyui-numberbox" required="true" /><input type="button" onclick="validUserId()" value="校验ID"></input>
		    	</td>
			  </tr>
			  <tr>
			 ...
			 <tr>
			  ...
			  <tr>
			    	...
			    	<td>
			    		<textarea  id="entryDescContent" name="entryDesc" class="easyui-validatebox" required="true"  data-options="validType:['length[1,500]']"   maxlength="500">${dto.entryDesc }</textarea>
			    		<p> 
			  ...
			  <tr>
			    	...
			    	<td>
			     		<textarea  id="titleDescContent" name="titleDesc" rows="3" cols="" class="easyui-validatebox" required="true"  data-options="validType:['length[1,500]']"   maxlength="500">${dto.titleDesc }</textarea>
			     		<p> 
			  ...
			  <tr>
			    	...
			    	<td>
			     		<textarea  id="detailDescContent" name="detailDesc" rows="3" cols="">${dto.detailDesc }</textarea>
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">服务案例：</th>
			    	<td>
			     		<textarea  id="serviceCaseContent" name="serviceCase" rows="3" cols="">${dto.serviceCase }</textarea>
			    	</td>
			  </tr>
			  <tr>
			  ...
			  <tr>
				 ...
				 <td>
					<div  style="margin-top: 10px;margin-bottom: 10px;">
						  <button type="button" class="btn btn-primary"  onclick="updateEntryPrise()">保存</button>&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<input type="hidden" name="isGood" value="${dto.isGood }"/>
...
//初始化加载分类
function initClass(){
	var entryPrise_sel_Class_content = $("#entryPrise_sel_Class_content")
	var initClass = ${entryClasses};
	var initClassHtml = '';
	if(initClass){
		$.each(initClass, function(i, n){
			initClassHtml += "<button onclick='delPic(this)' class='up_sel_class' type='button' id='"+n.class_id+"'>"+n.className+"</button>&nbsp;&nbsp;";
		});
	}
	entryPrise_sel_Class_content.html(initClassHtml);
}
//初始化加载标签
function initTags(){
	var entryPrise_sel_tags_content = $("#entryPrise_sel_tags_content");
	var initTags = ${entryTags};
	var initTagsHtml = '';
	if(initTags){
		$.each(initTags, function(i, n){
			initTagsHtml += "<button onclick='delPic(this)' class='up_sel_tag' type='button' id='"+n.tagId+"'>"+n.tagName+"</button>&nbsp;&nbsp;";
		});
	}
	entryPrise_sel_tags_content.html(initTagsHtml);
}
//初始logo图片
function initLogoPic(){
	var logoPicUrl = '${dto.logoImgPath}';
...
//初始认证图片
function initReconPic(){
	var recons = ${recons};
	if(recons){
		$("#reconImgs").html("");
		$.each(recons, function(i, n){
			if($.trim(n.imgPath).length > 0){
				var imgHtml = "<img title='点击删除' onclick='delPic(this)'   srcpath='"+n.imgPath+"' src='"+_oss_url+n.imgPath+"' class='up_pic_img_recon' style='width:55px; height:10px;'/>";
			}
...
//初始banner图片
function initBannrPic(){
	var banners = ${banners};
	if(banners){
		$("#bannerImgs").html("");
		$.each(banners, function(i, n){
			if($.trim(n.imgPath).length > 0){
				var imgHtml = "<img title='点击删除' onclick='delPic(this)'   srcpath='"+n.imgPath+"' src='"+_oss_url+n.imgPath+"' class='up_pic_img_banner' style='width:375px; height:180px;'/>";
			}
...
function initPhone(){
	...
	if(phoneLength > 0){
		if(phoneLength == 1){
			phoneString += '<input type="text" name="1" id="1" value="'+phones[0]+'" class="easyui-validatebox" required="true" /><img id="1" src="<%=basePath%>/resource/images/qfy/jia32.png" onclick="addTr(this)"/>';
			$(".phoneClass").html(phoneString);
		}else{
			for(var i = 0;i<phones.length;i++){
				if(i==0){
					...
					phonejianString += '<tr><td class="phoneClass"><input type="text" value="'+phones[i]+'" class="easyui-validatebox" required="true" /><img id="'+i+1+'" src="<%=basePath%>/resource/images/qfy/jian32.png" onclick="delTr(this)"/></td></tr>';
				}
			}
		}
	}
	$(".phoneClass").parent().parent().append(phonejianString); 
}
...
var options2 = {
		...
		items:['source', '|', 'fullscreen','undo', 'redo', '|', 'justifyleft', 'justifycenter', 'justifyright',
...
// 	选择标签
	...
	if(!validateMobileFlag){
	 ...
	 if(flag){
		if(!validateSubmitForm($('#entryPrise_update_form'))){
			return;
		}
	}
}
