...
<div style="width: 800px;margin: 0 auto;" >
	<form  action="news/update.do"   method="post" id="news_form"  afterCallback="saveCallback" enctype="multipart/form-data" >
		  ...
		  <input id="isHot" name="isHot" value="${n.isHot}"  type="hidden" />
		  <input id="lookCount" name="lookCount" value="${n.lookCount}"  type="hidden" />
		  
		 <table class="table table-nobordered " style="margin-top: 25px;">
			  <tr>
			    	<th style="width: 80px">资讯标题：</th>
			    	<td>
			     		<input type="text" name="newsTitle" class="easyui-validatebox" data-options="required:true" data-options="validType:['length[1,30]']"  value="${n.newsTitle}"  maxlength="30"/>
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">资讯分类：</th>
			    	<td>
					<a href="news/lookUpPage.do"  lookupGroup="newsclass" title="分类查询">
							<input type="text" readonly="readonly"  toName="newsclass.name"  class="easyui-validatebox" required="true" value="${n.tagName}"/>
					</a>
						<input type="hidden" name="types"  toName="newsclass.id"  value="${n.types}"/>
			    	</td>
			  </tr>
			   <tr>
			    	<th style="width: 80px">摘自来源：</th>
			    	<td>
			     		<input type="text" name="source" class="easyui-validatebox" required="true" data-options="validType:['length[1,20]']"  maxlength="20"  value="${n.source}" />
			    	</td>
			    	<td>&nbsp;</td>
			  </tr>
			  <tr>
				...
				<td>
					<c:choose>
						<c:when test="${published eq 0 }">
							<input type="text" name="createTime" id="createTime" value="<fmt:formatDate  value="${n.createTime}" pattern="yyyy-MM-dd HH:mm:ss" />"  maxlength="50" class="easyui-validatebox" required="true"    onFocus="WdatePicker({skin:'whyGreen',startDate:'%y-%M-%d %H:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss'})" />
						</c:when>
						<c:otherwise>
							<input type="text" name="createTime" id="createTime" value="<fmt:formatDate  value="${n.createTime}" pattern="yyyy-MM-dd HH:mm:ss" />"  maxlength="50" class="easyui-validatebox" required="true"  readonly="readonly"   />
						</c:otherwise>
					</c:choose>
				</td>
			  </tr>
			  ...
			  <%--  <tr>
			    	<th style="width: 80px">收費J币：</th>
			    	<td >
			     		<input type="text" name="charge" class="easyui-numberbox" value="${n.charge}"  maxlength="10" />
			    	</td>
			  </tr> --%>
			  <tr>
			    	<th style="width: 80px">评论数：</th>
			    	<td >
			     		<input type="text" name="commentCount"  id="commentCount" value="${n.commentCount}" class="easyui-numberbox"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">阅读人数：</th>
			    	<td >
			     		<input type="text" name="lookCount"  id="lookCount" value="${n.lookCount}" class="easyui-numberbox"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
				 <th></th>
		  ...
		  <input type="hidden" name="updateTime" id ="updateTime" value="<fmt:formatDate  value='${n.updateTime}' pattern='yyyy-MM-dd HH:mm:ss' />" />
