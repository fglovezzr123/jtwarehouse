					...
					success: function(data,res){
						...
						$.each(data,function(index){
							...
							if(honor_flag=="honor_001"){
								tjIdImg = '<div style="flex: 1; margin-left: .1rem;margin-top: -0.1rem;"><img src="${path}/resource/img/icons/tjjr.png"  class="tj" /></div>'
							}else if(honor_flag=="honor_002"){
								tjIdImg = '<div style="flex: 1; margin-left: .1rem;margin-top: -0.1rem;"><img src="${path}/resource/img/icons/tjyq.png"  class="tj" /></div>'
							}else if(honor_flag=="honor_003"){
								tjIdImg = '<div style="flex: 1; margin-left: .1rem;margin-top: -0.1rem;"><img src="${path}/resource/img/icons/tjhb.png"  class="tj" /></div>'
							}else if(honor_flag=="honor_004"){
								tjIdImg = '<div style="flex: 1; margin-left: .1rem;margin-top: -0.1rem;"><img src="${path}/resource/img/icons/tjfws.png"  class="tj" /></div>'
							}
							var str = '<div class="info active_A infoId " data_id="'+data[index].id+'" data_user="'+data[index].follow_user+'">'+
							             '<div class="info-l"><img src="'+data[index].head_portrait+'" class="p-img" />'+
							               '<div class="person">'+
							               '<div style="display: flex;">'+
							                '<span class="p-name">'+data[index].nickname+'</span>'+
							                tjIdImg+
							              '</div>'+
							                '<span class="company">'+data[index].job_name+'&nbsp;/'+data[index].com_name+'</span>'+
							                '</div></div><i class="iconfont info-R">&#xe60f;</i></div>';	
							$('.myF').append(str);
        				});
				...
				zdy_ajax({
					...
					success: function(data,res){
						...
						$.each(data,function(index){
							...
							var str = '<div class="info active_A" data_id="'+data[index].id+'" data_user="'+data[index].user_id+'">'+
							            '<div class="info-l"><img src="'+data[index].head_portrait+'" class="p-img" />'+
								           '<div class="person">'+
								              '<div style="display: flex;">'+
								                '<span class="p-name">'+data[index].nickname+'</span>'+
