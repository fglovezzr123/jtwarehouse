...
<script type="text/javascript">       
	   var userId = '<%=userid%>';
	   var oUserId=imPrefixion+ '<%=userid%>';
	var qid = window.location.href.split('=')[1];
	var oQid= imPrefixion+window.location.href.split('=')[1];
	// XMPP服务器BOSH地址
	...
	function del() {
		var datas='';
		var newGid=sessionStorage.getItem('groupNo163');
		 ...
		 if(myStorage){
			...
			if(datas[newGid]){
				...
				sessionStorage.setItem('groupNo163','0');
			}
		...
		success : function(data, res) {
			...
			if (res.code == 0) {
					...
					$('.sr').css({
					 ...
					 //群主修改群名
					   $('.xgqm').on('click',function(){
	        	             window.location.href = _path + "/wx/businessFriend/xgqm.do?qid=" + oQid;
	                  })
	                  //群主修改群标签
	                   $(".uList").on('click', function() {
				          window.location.href = _path + "/wx/businessFriend/GroupLabel.do?qid1=" + oQid;
	                    })
				} else {
					k1 = false
					$('.sr').css({
				...
				$.each(data.users,function(index) {
					...
					if (data.users[index].user_id == oUserId) {
	...
	function yaoUser(arrUserId){
		$(".jr").on('click',function() {
			...
			window.location.href = _path+ "/wx/businessFriend/myFriend.do?qid1=" + oQid
		})
	}
	...
	$(".sr").on('click',function() {
				window.location.href = _path+ "/wx/businessFriend/shanren.do?qid2=" + oQid
			})
	//客服留言
