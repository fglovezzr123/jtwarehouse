...
public class ListValuesAction extends BaseAction{
	...
	@RequiresPermissions("list:add")
	@RequestMapping("add")
	public ModelAndView add(ListValues lv,Errors errors){
		if(errors.hasErrors()) {  
	...
	public ModelAndView update(ListValues lv,Errors errors){
		if(errors.hasErrors()) {  
			ModelAndView mav=getValidationMessage(errors);
			if(mav!=null)return mav;
        }
	...
	public ModelAndView getSelectList(Integer type){
		...
		if(type!=null){
			List<ListValues> values=service.selectListByType(type);
			if(!values.isEmpty()){
