...
--%>
	...
	<form  action="entryServiceTag/update.do"  id="entryServiceTag_form"  method="post"   onsubmit="return validateSubmitForm(this)">
	    <input id="imgPath" name="imgPath" value="${dto.imgPath}"  type="hidden" />
		<div class="divider"></div>
		  <table class="table table-nobordered " style="margin-top: 25px;">
		 
		 	  <tr>
			    	<th style="width: 120px">标签图片：</th>
			    	<td >
                		<span id="updateServiceTagImgs" title="点击删除" ></span>
			  ...
			  <tr>
			    	...
			    	<td>
			     		<input type="text" name="name" class="easyui-validatebox" value="${dto.name}" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
				 <th></th>
		  ...
		  <input type="hidden" name="hotEntryId" value="${dto.hotEntryId}"/>
		  <input type="hidden" name="hotSort" value="${dto.status}"/>
...
if($.trim(picUrl).length > 0){
	$("#updateServiceTagImgs").html("");
	var imgHtml = "<img title='点击删除' onclick='delPic(this)'   srcpath='"+picUrl+"' src='"+_oss_url+picUrl+"' class='up_pic_img' style='width:200px; height:200px;'/>";
	$("#updateServiceTagImgs").append(imgHtml);
}
