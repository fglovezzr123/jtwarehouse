 	 ...
 	 <%----%><input value="${bean.id}" name="id" type="hidden" />
		<table class="table table-nobordered " style="margin-top: 25px;">
			<tr>
				...
				<td>
					<input type="text" name="productName" value="${bean.productName}" class="easyui-validatebox" required="true" data-options="validType:['length[1,30]']"
					 maxlength="30" />
				</td>
				<th style="width: 120px">权重：</th>
				<td>
					<input type="text" name="ratio" value="${bean.ratio}" class="easyui-validatebox" required="true" data-options="validType:['length[1,30]']" maxlength="30"
					/>
				</td>
			</tr>
			<tr>
					...
					<%--<input type="text" name="orgName" class="easyui-validatebox" required="true"  data-options="validType:['length[1,30]']"   maxlength="30" />--%>
						<select name="orgNo" id="orgNo" required="true" style="width: 120px;">
							<c:forEach items="${org}" var="item">
								<option value="${item.id}" <c:if test="${item.orgNo==bean.orgNo}">selected</c:if>>${item.orgName}</option>
							</c:forEach>
				...
				<td>
					<select name="isShow" id="isShow" required="true" style="width: 120px;">
						<option value="1" <c:if test="${bean.isShow==1}">selected</c:if>>是</option>
						<option value="2" <c:if test="${bean.isShow==2}">selected</c:if>>否</option>
					</select>
				</td>
			</tr>
			<tr>
				<th style="width: 120px">投资类型：</th>
				<td>
					<select name="investType" id="investType" required="true" style="width: 120px;">
						<option value="1" <c:if test="${bean.investType==1}">selected</c:if>>阳光私募</option>
						<option value="2" <c:if test="${bean.investType==2}">selected</c:if>>公募基金</option>
						<option value="3" <c:if test="${bean.investType==3}">selected</c:if>>信托计划</option>
						<option value="4" <c:if test="${bean.investType==4}">selected</c:if>>资管计划</option>
					</select>
				</td>
			...
			<tr>
				<th style="width: 120px">收益率：</th>
				<td>
					<input type="text" name="expectedRate" value="${bean.expectedRate}" style="width: 40px;" />%
				</td>
			</tr>
			<tr>
				<th style="width: 120px">产品状态：</th>
				<td>
			...
			<tr>
				<th style="width: 120px">认购起点：</th>
				<td>
					<input type="text" name="startPoint" value="${bean.startPoint}"/>
				</td>
			</tr>
			<tr>
				<th style="width: 120px">起购日期：</th>
				<td>
					<input type="text" name="placeDate" value="${bean.placeDate}"/>
				</td>
			</tr>
			<!-- <tr>
						<th style="width: 120px">金额范围：</th>
						<td>
							<input type="text" name="financeAmount1"  style="width: 40px;"/>-<input type="text" name="financeAmount2"  style="width: 40px;"/>万
						</td>
					</tr>
					<tr>
						<th style="width: 120px">期限范围：</th>
						<td>
							<input type="text" name="financePeriod1"  style="width: 40px;"/>-<input type="text" name="financePeriod2" style="width: 40px;"/>月
						</td>
					</tr>
					<tr>
						<th style="width: 120px">申请条件：</th>
						<td>
							<textarea id="conditions" name="conditions" rows=5" cols="35"></textarea>
						</td>
					</tr> -->
			<tr>
				<th style="width: 120px">募集情况介绍：</th>
				<td>
					<textarea id="introduce" name="introduce" rows="5" cols="35">${bean.introduce}</textarea>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<textarea name="productDetail" style="width:320px;height:600px;" id="productDetail">${bean.productDetail}</textarea>
				</td>
			</tr>
			<tr>
