...
<body>
	<div class="content active_A" id="personalIcon">
		<div class="content-item" onclick="setImg();">
			<div class="con-l">头像</div>
			<div class="con-r" style="background-image:url('')">
				<c:choose>
					<c:when test="${fn:indexOf(user.head_portrait,'http') ne -1 }">
						<img id="user_head" style="width:1.12rem;height:1.12rem;border-radius: 50%;" src="${user.head_portrait}" />
					</c:when>
					<c:otherwise>
						<img id="user_head" style="width:1.12rem;height:1.12rem;border-radius: 50%;" src="${ossurl}${user.head_portrait}" />
					</c:otherwise>
				</c:choose>
			</div>
	...
	<div class="content">
		...
		<div class="content-item bottom-border clear">
			...
			<!-- <div class="con-r active_A" id="con-r">
					<img src="${path}/resource/img/icons/edition.png" /> 编辑111
				</div> -->
			...
			zdy_chooseImg(function(data,res){
	    	 	if(res.code == 0){
	    	 		...
	    	 		zdy_ajax({
						...
						showLoading : false,
						data : {
							headPortrait : photos.img_url
						},
						success : function(data, res) {
							if (res.code == 0) {
								$("#user_head").attr("src",photos.img_url);
							} else {
								alert(res.msg);
							}
						},
