...
<html lang="en">
	...
	<body style="background: #f2f3f4;>
		<div class="Z-register-login" >
			...
			<div class="gsj">
			<span class="holder" style="text-align:left;line-height:0.5rem;font-size:0.26rem">评论不得超过200字</span>
				<div contenteditable="true" class="inText"  id="cmeDesc"></div>
				<!--  <textarea id="cmeDesc" name="cmeDesc"  placeholder="评论不得超过200字" class="nerong"  maxLength=200  onpaste="value=clearYEmoil1(value)" oncontextmenu="value=clearYEmoil1(value)" >${comment.cmeDesc}</textarea>--> 
		...
		<script type="text/javascript">
				$(".inText").bind("focus",function(){
					$(".holder").hide();	
				})
				$(".inText").bind("blur",function(){
					if($(this).text()==''){
						$(".holder").show();
					}else{
						if($(this).text().length>200){
							...
							$("#dyContentLength").text($(this).text().length+'/200');
						}
					}
			...
			//粘贴时，触发
				$(".inText").bind("paste",function(){
					$("#dyContentLength").text($(this).text().length+'/200');
				})
			//剪切时，触发
				$(".inText").bind("cut",function(){
					$("#dyContentLength").text($(this).text().length+'/200');
					if($(this).text()==''){
						$(".holder").show();
					}
			...
			function getByteLen(val) {
				...
				$("#cmeDesc").on("blur",function(){
					$("#cmeDesc").text(clearYEmoil1($("#cmeDesc").text()));
