...
<body style="background: #ebebeb">
  ...
  <%--   <script src="${path}/resource/js/NIM_Web_Chatroom_v4.1.0.js"></script>
	...
	var pgname=JSON.parse(sessionStorage.getItem('user_info'));
    $('title').html(pgname.nickName);
	var data = {};
	var myheadpath='';
	var historytimes='';
	var myScroll;
	var lastMSGid='';
	var oldgroudId=window.location.search.split("=")[1];
	var myaccounts='<%=userid %>',groupid=sessionStorage.getItem('groupNo163');
	var omyaccounts=imPrefixion+'<%=userid %>',oGroupid=sessionStorage.getItem('groupNo163');
	var scrolled=false;
	...
	zdy_ajax({
	  ...
	  function onMsg(msg) {
	      ...
	      if(msg.scene=="team" && msg.to==oGroupid){
	    	  var varId=yzm();
    		  if(msg.fromClientType=="Web"){
    			  if(msg.content){
    	    		  var urls=JSON.parse(msg.content)
    	    		  if(urls.type==2){
    	    			  $('#scroller').append('<div class="yourFriend" id="'+varId+'" ><img src="'+heads.headparth+'" class="_head"  onclick="ckInfo1('+msg.from+')"/><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls.data.url+'"/></div>' + '</span></div><div  class="poles" style="height:2.5rem;"></div>'); 
    	    		  }else if(urls.type==3){
    	    			  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+msg.from+')" /><span style="line-height:0.5rem;display:flex;">'+ '<img class="faudios" style="width:0.35rem;height:0.5rem;" FR="1" count="'+urls.data.times+'" onclick="fplayvoidce(this)" audio="'+urls.data.url+'" src="${path}/resource/img/images/FFFFFFFFFFFF.png"><span style="padding:0">'+urls.data.times+'\" '+'</span></span></div>');
    	    		  }
    	    		}else{
    	    			 $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+msg.from+')" /><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",msg.text) + '</span></div>');
    	    		}
    		  }else{
    			  if( msg.type=="text"){
    	    		  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+msg.from+')"/><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",msg.text) + '</span></div>');
    	    	  }else if(msg.type=="image"){
        			  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+msg.from+')"/><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+msg.file.url+'"/></div>' + '</span></div><div  class="poles" style="height:2.5rem;"></div>'); 
	  ...
	  function saveLocalSessions(pushID,session,cur){
		    ...
		    if(session.lastMsg.fromClientType=="Server" && session.scene=="team"){
		    	console.log("服务器信息");
		    	if(myStorage){
		    		var dataJsons=JSON.parse(myStorage);
		    		if(dataJsons[session.to] && session.teamName){
		    			dataJsons[session.to]["teamName"]=session.teamName;
		    			localStorage.setItem("recentContacts",JSON.stringify(dataJsons));
		    		}
		    	}
		    	return;
		    };
				   ...
				   nim.getUser({
				     ...
				     nim.getTeam({
				    	    teamId: pushID,
					    	    ...
					    	    console.log(obj);
					    	    datas[pushID]=session;
					    	    datas[pushID]["teamName"]=obj.name;
								 if(pushID==oGroupid){
...
//文本发送	
	  $("#sendTexts").click(function(e){
		  ...
		  var msg = nim.sendText({
			    ...
			    custom:JSON.stringify({headparth:data.myInfo.avatar,oldid:oldgroudId}),
			    needPushNick:true
			});
			...
			function sendMsgDone(error, msg) {
			    ...
			    if(!error){
			    	 $('#scroller').append('<div class="myself" id="'+varId+'"><span>' +  $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",$("#t-content").val()) + '</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')"/></div>');
		  ...
		  var msg = nim.sendCustomMsg({
		      ...
		      custom:JSON.stringify({headparth:data.myInfo.avatar,oldid:oldgroudId}),
		    	  ...
		    	  if(!error){
		    		  ...
		    		  var varId=yzm();
		    		   if(ty==2){
			    		$('#scroller').append('<div class="myself "><span>'+'<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls+'"/>' + '</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')"/></div><div id="'+varId+'" class="poles" style="height:2.5rem;"></div>');
		    		   }else{
		    			   $('#scroller').append('<div class="myself" id="'+varId+'"><span style="display:flex;line-height:0.5rem;"><span style="padding:0">' +time+'\" '+'</span><img class="audios" FR="0" style="width:0.35rem;height:0.5rem;" count="'+time+'" onclick="playvoidce(this)" audio="'+urls+'" src="${path}/resource/img/images/MMMMMMMMMMMM.png">'+'</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')"/></div>');   
		    		   }
		    		   if($("#scroller").height()<$(window).height()*0.8){
							$(".poles").remove();
							myScroll.refresh();
	...
	function startRecord (){
		...
		wx.startRecord({
			  success:function(res){
				  $("#pressRecord").addClass('startrecod');
				  $("#pressRecord").text('结束录音');
		          $("#recordIcon").show();
		          t_seconds=0;
		  		  timedCount();
			  },
			    ...
			    endTime:timestamp,
				    ...
				    if (!error) {
				        ...
				        for(var i=0;i<obj.msgs.length;i++){
				        	 ...
				        	 if(obj.msgs[i].from==omyaccounts){
					    		  if(obj.msgs[i].type=="text" || obj.msgs[i].type=="custom"){
					    			  if(obj.msgs[i].content){
					    				  var urls=JSON.parse(obj.msgs[i].content);
					    	    		  if(urls.type==2){
					    	    			  $('#scroller').prepend('<div class="myself "><span>'+'<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls.data.url+'"/>' + '</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')" /></div>'); 
					    	    		  }else{
					    			  ...
					    			  $('#scroller').prepend('<div class="myself "><span>'+'<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+obj.msgs[i].file.url+'"/>' + '</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')" /></div>'); 
				        		 ...
				        		 if(obj.msgs[i].type=="text" || obj.msgs[i].type=="custom"){
					    			  if(obj.msgs[i].content){
					    				  var urls=JSON.parse(obj.msgs[i].content);
					    	    		  if(urls.type==2){
					    	    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+obj.msgs[i].from+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls.data.url+'"/></div>' + '</span></div>'); 
					    	    		  }else{
					    	    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+obj.msgs[i].from+')" /><span style="line-height:0.5rem;display:flex;">'+ '<img class="faudios" style="width:0.35rem;height:0.5rem;" FR="1" count="'+urls.data.times+'" onclick="fplayvoidce(this)" audio="'+urls.data.url+'" src="${path}/resource/img/images/FFFFFFFFFFFF.png"><span style="padding:0">'+urls.data.times+'\" '+'</span></span></div>');
					    	    		  }
					    	    	  }else if(obj.msgs[i].type=="text" && !obj.msgs[i].content){
					    	    		  $('#scroller').prepend('<div class="yourFriend"><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+obj.msgs[i].from+')" /><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",obj.msgs[i].text) + '</span></div>');
					    	    	  } 
					    		  }else if(obj.msgs[i].type=="image"){
					    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+heads.headparth+'" class="_head" onclick="ckInfo1('+obj.msgs[i].from+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+obj.msgs[i].file.url+'"/></div>' + '</span></div>'); 
					    		  }else if(obj.msgs[i].type=="audio"){
	...
	function pullDownAction(stamp) {
		...
		//最近联系人未读条数初始化
		//saveLocalSession(groupid,'',true);
		$('.sz').on('click', function() {
			if($("#recordIcon").css('display')=='block'){
	    		layer.msg("请先结束录音"); 
	    		return;
	    	}
				window.location.href = _path + "/wx/businessFriend/msg.do?follow_user="+oldgroudId;
			})
		...
		function ckInfo1(to){
			window.location.href = _path+ "/wx/businessFriend/friendInfo.do?follow_user="+to;	 	
	     }
	</script>
</body>
</html>
