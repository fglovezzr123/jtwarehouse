...
public class FrontKeyFilter implements Filter {
...
class XssRequestWrappery extends HttpServletRequestWrapper {
	WebApplicationContext wac = ContextLoader.getCurrentWebApplicationContext();
	IRefundInstructionService refundInstructionService =wac.getBean(IRefundInstructionService.class);
	...
	public XssRequestWrappery(HttpServletRequest request) {
		super(request);
	}
	...
	public String doFilter(String value) {
		...
		RefundInstruction re = refundInstructionService.getrefundinstructionByid(3);
		String str="";
		if(re!=null){
			str = re.getContent();
		}
		...
		for(String s : strlist){
			value=value.replaceAll(s, returnstar(s)); //屏蔽敏感词
		}
	...
	private String returnstar(String sensitive){
		String s ="";
