...
function loadMeetingWhitelist(){
	$("#MeetingWhitelist_datagrid").datagrid({
		url : "meeting/meetingwhitelistquery.do",
...
function addMeetingWhitelist(){
	...
	MUI.openDialog('添加','meeting/addwhitelistpage.do?meetingId=${meetingId}',"MeetingWhitelist_add",params);
}
function delMeetingWhitelist(){
	...
	if(rows&&rows.length==0){
		...
		Msg.confirm('提示',"确定要删除吗？<br/>", function(r){
          	if (r){
          		$.ajax({
    				url:"meeting/whitelistpage/del.do",
    				type: 'post',	
    				data: {ids: arr.join(";")},
    				cache: false,
    				dataType:"json",
