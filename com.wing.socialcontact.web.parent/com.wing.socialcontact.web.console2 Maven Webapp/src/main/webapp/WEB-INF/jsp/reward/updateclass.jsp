...
<%--
	模块：系统管理--悬赏 行业管理--编辑分类
--%>
<div style="width: 600px;margin: 0 auto;" >
	<form  action="reward/updateClass.do"   method="post"   id="news_form" afterCallback="saveCallback" enctype="multipart/form-data">
		  <input id="imagePath" name="imagePath" value="${r.imagePath}"  type="hidden" />
		 ...
		 <table class="table table-nobordered " style="margin-top: 25px;">
			  <tr>
			    	<th style="width: 80px">行业名称：</th>
			    	<td>
			     		<input type="text" name="className" class="easyui-validatebox"   value="${r.className}" required="true"  data-options="validType:['length[1,20]']"   maxlength="20"/>
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">序号：</th>
			    	<td>
			     		<input type="text" name="classNum" value="${r.classNum}"  class="easyui-numberbox" required="true"    maxlength="3" max="999"/>
			    	</td>
			  </tr>
			  <tr>
			    	...
			    	<td >
						...
						<option  value="1" <c:if test="${r.isRecommend==1}">selected</c:if>>推荐</option>
						<option  value="2"  <c:if test="${r.isRecommend==2}">selected</c:if>>不推荐</option>
					    </select>   	
                	</td>
			  </tr>
			   <tr>
		  ...
		  <input type="hidden" name="createTime" value="<fmt:formatDate  value='${r.createTime}' pattern='yyyy-MM-dd HH:mm:ss' />" />
