	...
	<form  action="news/update1.do"   method="post"   id="news_form"  afterCallback="saveCallback" enctype="multipart/form-data" >
		 ...
		 <table class="table table-nobordered " style="margin-top: 25px;">
			  <tr>
			    	...
			    	<td>
			     		<input type="text" name="newsTitle" class="easyui-validatebox" data-options="required:true"  value="${n.newsTitle}"  maxlength="50" data-options="validType:['length[1,50]']"/>
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">链接：</th>
			    	<td>
			     		<input type="text" name="url"  id="url" class="easyui-validatebox"  value="${n.url}" required="true"  data-options="validType:['length[1,100]']"   maxlength="100" style="width:500px;"/>
...
function save(){
	...
	if(jumpUrl!=''){
		if(jumpUrl.indexOf("http://")!=0){
			Msg.alert("提示","链接以 http:// 开头！","warning",null);
			return false;
		}
	}
	...
	if(iscontents<= 0){
		Msg.alert("提示","请填写摘要！","warning",null);
    	return;
	}
	if(flag){
