...
public class CommUtil {
	...
	public static void main(String[] strs) {
		...
		// CommUtil.sendSMS(mobile, content);
		// CommUtil.sendSMS("15801276745",
		// "客户您好，您有一个待付款订单还未支付，1小时内未支付订单将被取消，请尽快支付，感谢惠顾。");
		// CommUtil.sendSMS("13439902517",
		// "客户您好，您有一个待付款订单还未支付，1小时内未支付订单将被取消，请尽快支付，感谢惠顾。");
		// sendMobileCode("13760262786");
		// System.out.println(getIpAddr());
		System.out.println(formatNumStr(1232111111.0));
	}
	...
	public static String getTimesToNow(String date) {
		...
		try {
			long from = df.parse(date).getTime();
	...
	public static String getTimesToNow2(String date) {
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String now = df.format(new Date());
		String returnText = null;
		try {
			...
			if (days == 0) {// 一天以内，以分钟或者小时显示
				...
				returnText = date.substring(0, date.lastIndexOf(":"));
			}
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static String formatChineseDate(Object v) {
		if (v == null)
			return null;
		SimpleDateFormat df = new SimpleDateFormat("MM月dd号");
		return df.format(v);
	}

	public static int formatMonth(Object v) throws ParseException {
		if (v == null)
			return 0;
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String df = sdf.format(v);
		/// System.out.println(df);
		Date date = sdf.parse(df);
	...
	public static long getDateDistance(String time1, String time2) {
		...
		try {
			...
			quot = quot / 1000L / 60L / 60L / 24L;// 天差
			// quot = quot / 1000L / 60L / 60L ;//小时差
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return quot;
	...
	public static void listSortBySequence(List list) {
		if (list == null)
			return;
		Collections.sort(list, new Comparator() {
			public int compare(Object obj1, Object obj2) {
				if (obj1 == null || obj2 == null) {
					...
					try {
						int seq1 = (int) obj1.getClass().getMethod("getSequence").invoke(obj1);
						int seq2 = (int) obj2.getClass().getMethod("getSequence").invoke(obj2);
						if (seq1 == seq2) {
							return 0;
						}
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static void listSortById(List list) {
		if (list == null)
			return;
		Collections.sort(list, new Comparator() {
			public int compare(Object obj1, Object obj2) {
				if (obj1 == null || obj2 == null) {
					...
					try {
						int seq1 = (int) obj1.getClass().getMethod("getId").invoke(obj1);
						int seq2 = (int) obj2.getClass().getMethod("getId").invoke(obj2);
						if (seq1 == seq2) {
							return 0;
						}
						return seq1 < seq2 ? 1 : -1;
	...
	public static BigDecimal parse2Num(Object obj, boolean isZERO) {
		try {
	...
	public static String formatNumStr(double d) {
