...
<form method="post" action="district/update.do" onsubmit="return validateSubmitForm(this)">
	...
	<table class="table table-bordered ">
			<tr>
				...
				<td>
					<input type="text" name="disSort" value="${requestScope.d.disSort }" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"class="easyui-numberbox" required="true"  min="1" max="9999" maxlength="4"/>
