...
public class SystemLogAction extends BaseAction{
	...
	public String loginExport(String[] ids,HttpServletRequest request,HttpServletResponse response){
		...
		//return ExcelUtils.export(service.selectSyLoginLogs(ids), service.findTableCustomExport((short)1), "登录日志导出.xls", request, response);
		return null;
	}
	/**
	 * 打印登录日志
	...
	public String loginExport(String[] ids,ModelMap map){
		...
		map.addAttribute("tc",new ArrayList());
		
		map.addAttribute("tableTitle", "登录日志");
