...
<div class="search-div" id="toolbar">
	<div class="search-content">
		...
		<span>
			<button class="btn btn-primary btn-small" type="button" onclick="addDynamic()">发布动态</button>&nbsp;
		</span>
	</div>
</div>
		...
		columns : [[
			...
			},
			{
				...
				formatter: function(value, row, index){
							...
							if(isEmpty(row.media_id)&&isEmpty(row.articleid)&&row.dy_type=='0'){
								str +="<a href = 'javaScript:void(0)' onclick = editDynamic('"+row.id+"')>编辑</a>&nbsp;|&nbsp;";
							}
							if(row.is_stick==1){
...
function editDynamic(fid){
	var params = { onClose:function(){  
        $("#dynamiceditpage").dialog('destroy');  
    }, closed: false,cache: false,modal:true,width:800,height:400,collapsible:false,minimizable:false,maximizable:false};
	MUI.openDialog('编辑动态','dynamic/dynamicedit.do?id='+fid,"dynamiceditpage",params)
}
</script>