...
function initWebUpload(){
	...
	uploader.on("uploadSuccess", function( file, response ) {
		if(response.code==0){
			if(imgType=="hb"){
				...
				$("#hb1").attr("src",response.dataobj.img_url+"?x-oss-process=style/YSMAX640");
				$("#hb1").attr("status","1");
	...
	uploader.on('error', function(handler) {
		if(handler=="F_EXCEED_SIZE"){
			alert("请上传小于2M的PNG、JPG、JPEG图片，建议尺寸为"+(imgType==="guest"?"100*100":"640*320"))
			return;
