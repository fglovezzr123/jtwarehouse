...
public class TableCustomAction extends BaseAction{
	...
	public ModelAndView add(SyTableCustom tc,Errors errors){
		if(!ServletUtil.isDeveloper()){
			return ajaxDoneError(MsgConfig.MSG_KEY_NOPOWER);
		}
		if(errors.hasErrors()) {  
	...
	public String devUpdatePage(String id,ModelMap map){
		...
		SyTableCustom tc=service.selectByPrimaryKey(id);
		if(tc==null){
			return NODATA;
		}
	...
	public ModelAndView devUpdate(SyTableCustom tc,Errors errors){
		if(!ServletUtil.isDeveloper()){
			return ajaxDoneError(MsgConfig.MSG_KEY_NOPOWER);
		}
		if(errors.hasErrors()) {  
	...
	public ModelAndView update(SyTableCustom tc,Errors errors){
		if(errors.hasErrors()) {  
