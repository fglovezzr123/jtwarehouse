	...
	<form  action="club/update.do"  method="post" id="club_form" afterCallback="saveCallback" enctype="multipart/form-data" >
		  ...
		  <table class="table table-nobordered " style="margin-top: 25px;">
			  ...
			  <tr>
			    	...
			    	<td >
			     		<input type="text" name="title"  readonly="readonly" value="${dto.title }" class="easyui-validatebox" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">会所地址：</th>
			    	<td >
			     		<input type="text" name="address"  readonly="readonly"   value="${dto.address }"  class="easyui-validatebox" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			  <tr>
			    	<th style="width: 80px">会所电话：</th>
			    	<td >
			     		<input type="text" name="phone"   readonly="readonly" value="${dto.phone }" class="easyui-validatebox" required="true"  data-options="validType:['length[1,20]']"   maxlength="20" />
			    	</td>
			  </tr>
			<tr>
					<th>分类：</th>
			  ...
			  <tr>
			    	...
			    	<td>
					<textarea id="content" readonly="readonly" name="content" style="width:320px;height:600px;">${dto.content }</textarea>
			    	</td>
			  </tr>
			   <tr>
...
function initUploadPicFrame(){
	var frameSrc = "<%=basePath%>club/getUploadPicForm.do";
...
function picUploadCallback(data){
	if (data.returnCode == "1"){
		...
		if(picUrl.length > 0){
			var imgHtml = "<img title='点击删除' onclick='delPic(this)' srcpath='"+data.picPath+"' src='"+oss+picUrl+"' class='up_pic_img' style='width:240px; height:160px;'/>";
    		$("#nimgs").append(imgHtml);
		}else{
			alert("上传失败,请稍后再试");
		}
