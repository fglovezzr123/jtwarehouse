			...
			<form onsubmit="return datagridSearch(this,'${param.rel }_datagrid');" >
				...
				<div class="search-toolbar">
					<span style="float:left;">
						...
						<a class="easyui-linkbutton" icon="icon-add" plain="true" href="pageConfig/addPage.do?rel=${param.rel }" title="新增聚合页面" target="dialog" width="700" height="400" rel="${param.rel}_add"><span>新增</span></a>
						<a class="easyui-linkbutton" icon="icon-edit" plain="true" href="pageConfig/updatePage.do?id={id}&rel=${param.rel }" target="dialog"  width="700" height="400" rel="${param.rel}_update" warn="请先选择一条记录" title="修改聚合页面" datagrid="${param.rel}_datagrid">修改</a>
						<a class="easyui-linkbutton" icon="icon-remove"	plain="true" href="pageConfig/del.do" target="selectedTodo"  title="确定要删除吗?" warn="至少勾选一条记录" datagrid="${param.rel}_datagrid">删除</a>
					...
					<form onsubmit="return datagridSearch(this,'pageColumn_datagrid');"  formatFilterData="true">
						<div class="search-toolbar">
							<span style="float:left;">
								...
								<a class="easyui-linkbutton" icon="icon-add" plain="true" href="pageConfig/pageColumn/addPage.do?rel=pageColumn" target="dialog" width="1000" height="550" rel="pageColumn_add" warn="请先选择一条主表记录" p_datagrid="${param.rel }_datagrid" title="新增栏目" type="2">新增</a>
								<a class="easyui-linkbutton" icon="icon-edit" plain="true" href="pageConfig/pageColumn/updatePage.do?id={id}&rel=pageColumn" target="dialog"  width="1000" height="550" rel="pageColumn_update" warn="请先选择一条记录" title="修改栏目" datagrid="pageColumn_datagrid">修改</a>
								<a class="easyui-linkbutton" icon="icon-remove"	plain="true" href="pageConfig/pageColumn/del.do" target="selectedTodo"  title="确定要删除吗?" warn="至少勾选一条记录" datagrid="pageColumn_datagrid">批量删除</a>
							</span>
							<span style="float:right;display: none;">
								<button class="btn btn-primary btn-small" type="submit">查询</button>&nbsp;
								<button class="btn btn-small clear" type="button">清空</button>&nbsp;
...
<script type="text/javascript">	
	...
	var load_pageColumn=function(pageId){
		$('#pageColumn_datagrid').datagrid({
			url : "pageConfig/pageColumn/query.do?pageId="+pageId,
			columns:[[
	            {
			    	field:"chk",
			    	title : "勾选",
			...
			onDblClickRow:function(rowIndex, rowData){
				MUI.openDialog('修改栏目','pageConfig/pageColumn/updatePage.do?id='+rowData.id+'&rel=pageColumn','pageColumn_update',{width:1000,height:550,buttons:[{
					text: '提交',
					iconCls: 'icon-ok',
					handler: function() {
						eval("save()");
