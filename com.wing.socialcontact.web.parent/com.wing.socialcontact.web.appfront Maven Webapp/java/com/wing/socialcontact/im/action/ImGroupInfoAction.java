...
public class ImGroupInfoAction extends BaseAction{
	...
	public @ResponseBody Map findMyGroupInfoListByUserId(HttpServletRequest request,String pageNum, String pageSize) throws IOException{
		...
		if(StringUtil.isEmpty(pageNum) || StringUtil.isEmpty(pageSize) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageNum) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageSize)){
			...
			try {
				List performs = imGroupinfoService.findMyGroupInfoListByUserId(userId, Integer.parseInt(pageNum), Integer.parseInt(pageSize),null);
	 ...
	 * @param imGroupinfo
	 * @param userIds
	 * @param isTop 1 置顶 0不置顶
	 * @param disturb 0可打扰 1、免打扰
	 * @param favIds
	 * @return
	 * @throws IOException
	 */
	@RequestMapping("insertMyGroupInfo")
	public @ResponseBody Map insertMyGroupInfo(HttpServletRequest request,ImGroupinfo imGroupinfo, String[] userIds,String isTop,String disturb,String[] favIds) throws IOException{
		...
		if(imGroupinfo == null || imGroupinfo.getGroupName() == null ||
				(imGroupinfo.getGroupType() != 1 && imGroupinfo.getGroupType() !=2) || isTop == null || disturb == null  
				|| (!"1".equals(isTop) && !"0".equals(isTop)) || (!"1".equals(disturb) && !"0".equals(disturb))
			...
			try {
				if(imGroupinfo.getHeadPortrait() == null){
					...
					String imgPath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/"+"resource/img/images/qun.png";
				...
				//创建群发送通知给IM服务器
				/*String result = OkhttpUtils.sendHttpRequest(ossConfig.getCreate_group_notify_url(),
						"{\n  \"id\": \""+groupId+"\"\n}",
				...
				if(userIds!=null&&userIds.length!=0){
					for(int i=0;i<userIds.length;i++){
						members.add(userIds[i]);
					}
				}
				tid = IMUtil.createTeamOne(imGroupinfo.getGroupName(), imGroupinfo.getMainUser(), members);
				imGroupinfo.setTid(tid);
				imGroupinfoService.updateGI(imGroupinfo);
				//此项年度积分增加上限为100
				userIntegralLogService.addLntAndEmp(userId, "task_0017");
				return super.getSuccessAjaxResult("创建成功！", groupId);
			} catch (Exception e) {
				return super.getAjaxResult(Constants.AJAX_CODE_ERROR_INFO, e.getMessage(), "");
			}
	...
	public @ResponseBody Map updateMyGroupInfo(HttpServletRequest request,ImGroupinfo imGroupinfo) throws IOException{
		...
		try {
			...
			/*String result = OkhttpUtils.sendHttpRequest(ossConfig.getCreate_group_notify_url(),
					...
					Constants.AUTHORIZATION);*/
			//通知IM
			IMUtil.updateTeam(imGroupinfo.getTid(), imGroupinfo.getGroupName(), imGroupinfo.getMainUser());
	...
	public @ResponseBody Map insertGroupRequest(HttpServletRequest request,String groupId,String askmessage) throws IOException{
	...
	public @ResponseBody Map updateGroupRequest(HttpServletRequest request,String requestId,String status) throws IOException{
		...
		if(StringUtil.isEmpty(requestId) || (!"2".equals(status) && !"3".equals(status))){
			...
			try {
				...
				if(status.equals("2")){
					...
					JSONArray members = new JSONArray();
					members.add(imGrequest.getUserId());
					IMUtil.addTeam(groupinfo.getTid(), groupinfo.getMainUser(), members);
	...
	public @ResponseBody Map selMyGroupRequest(HttpServletRequest request,String pageNum, String pageSize) throws IOException{
		...
		if(StringUtil.isEmpty(pageNum) || StringUtil.isEmpty(pageSize) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageNum) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageSize)){
			...
			try {
				List performs = imGrouprequestService.findMyGroupRequest(userId, Integer.parseInt(pageNum), Integer.parseInt(pageSize));
	...
	public @ResponseBody Map insertGroupUsers(HttpServletRequest request,String groupId,String[] userIds) throws IOException{
		...
		if(StringUtil.isEmpty(groupId) || userIds == null || userIds.length == 0 ){
			...
			try {
				...
				if(group != null){
					...
					IMUtil.addTeam(group.getTid(), group.getMainUser(), members);
	...
	public @ResponseBody Map delGroupUsers(HttpServletRequest request,String groupId,String[] userIds) throws IOException{
			...
			try {
				...
				if(group != null){
					...
					if(userId.equals(creator)){
						...
						if(userIds!=null&&userIds.length!=0){
							for(int i=0;i<userIds.length;i++){
								IMUtil.kickTeam(group.getTid(),  group.getMainUser(), userIds[i]);
							}
						}
	...
	public @ResponseBody Map updateGroupFavs(HttpServletRequest request,String groupId,String[] favIds) throws IOException{
	...
	public @ResponseBody Map updateGroupIsPrivate(HttpServletRequest request,String groupId,String status) throws IOException{
	 ...
	 * 设置是否置顶
	 * @param groupId
	 * @param status  1 置顶  0 取消置顶
	 * @return
	 * @throws IOException
	 */
	@RequestMapping("updateGroupIsTop")
	public @ResponseBody Map updateGroupIsTop(HttpServletRequest request,String groupId, String status) throws IOException{
	...
	public @ResponseBody Map updateGroupMsgDistrub(HttpServletRequest request,String groupId, String status) throws IOException{
				...
				if(group != null){
					...
					if(creator.equals(userId)){
						...
						/*String result = OkhttpUtils.sendHttpRequest(ossConfig.getCreate_group_notify_url(),
								...
								Constants.AUTHORIZATION);*/
						//通知IM
						IMUtil.leaveTeam(group.getTid(), userId);
					}
				}else{
					throw new RuntimeException("获取群组信息失败，请检查群ID信息。");
				}
			...
			try {
				...
				if(group != null){
					...
					if(creator.equals(userId)){
						...
						//创建群发送通知给IM服务器
						/*String result = OkhttpUtils.sendHttpRequest(ossConfig.getDelete_group_notify_url(),
								"{\n  \"id\": \""+groupId+"\"\n}",
								Constants.AUTHORIZATION);*/
						IMUtil.removeTeam(group.getTid(), group.getMainUser());
	...
	public  @ResponseBody Map selTopFriendListByGroupId(HttpServletRequest request,String groupId,String pageNum, String pageSize) throws IOException{
	
		String userId =  super.checkLogin(request);
		if(userId == null || "".equals(userId))	{
			return super.getAjaxResult(Constants.AJAX_CODE_ERROR_NOTLOGIN, "未登录，请重新登录！", "");
		}	
		if(StringUtil.isEmpty(groupId) || StringUtil.isEmpty(pageNum) || StringUtil.isEmpty(pageSize) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageNum) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageSize)){
			return super.getAjaxResult(Constants.AJAX_CODE_ERROR_PARAM, "参数无效，请检查！", "");
		}else{
			try{
				List performs = imToprelatService.findTopFriendListByGroupId(groupId, Integer.parseInt(pageNum),Integer.parseInt(pageSize));
				return super.getSuccessAjaxResult("获取成功！", performs);
			}catch(Exception e){
				return super.getAjaxResult(Constants.AJAX_CODE_ERROR_INFO, e.getMessage(), "");
			}
	...
	public @ResponseBody Map selGourpsListByName(HttpServletRequest request,String groupName,String pageNum, String pageSize) throws IOException{
		String userId =  super.checkLogin(request);
		if(userId == null || "".equals(userId))	{
			return super.getAjaxResult(Constants.AJAX_CODE_ERROR_NOTLOGIN, "未登录，请重新登录！", "");
		}	
		...
		if(StringUtil.isEmpty(pageNum) || StringUtil.isEmpty(pageSize) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageNum) || !com.wing.socialcontact.util.StringUtil.isNumeric(pageSize)){
			...
			try {
				List performs = imGroupinfoService.selGourpsListByName(userId,groupName,Integer.parseInt(pageNum), Integer.parseInt(pageSize));
