	...
	$("#meetingindex_datagrid").datagrid({
		...
		columns : [[
			...
			},
			{
				...
				width : 240,
				formatter: function(value, row, index){
					return  "<a href = 'javaScript:void(0)' onclick = showSignup('"+row.id+"')>报名用户</a>|"
...
function addMeeting(fid){
	...
	MUI.openDialog('发布会议','meetingApp/meetingaddpage.do?id=',"meetingaddpage",params)
}
//报名提醒用户
function showSignupRemind(fid){
	...
	MUI.openDialog('报名提醒用户','meetingApp/signupremind/index.do?meetingId='+fid,"meetingsingupremindpage",params)
}
//报名用户
function showSignup(fid){
	...
	MUI.openDialog('报名用户','meetingApp/signupindex.do?meetingId='+fid,"meetingsinguppage",params)
}
//白名单
function showWhitelist(fid){
	...
	MUI.openDialog('白名单','meetingApp/meetingwhitelist.do?meetingId='+fid,"meetingwhitelistpage",params)
}
//编辑
function updateMetting(fid){
	...
	MUI.openDialog('编辑会议','meetingApp/meetingupdatepage.do?id='+fid,"meetingaddpage",params)
}
//明细
function detailMetting(fid){
	...
	MUI.openDialog('会议明细','meetingApp/meetingdetail.do?id='+fid,"meetingdetailpage",params)
}
</script>