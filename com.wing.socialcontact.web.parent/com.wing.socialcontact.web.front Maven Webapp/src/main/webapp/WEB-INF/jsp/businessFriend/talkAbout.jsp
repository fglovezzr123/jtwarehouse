...
<body style="background: #ebebeb">
	...
	var oMyaccounts=imPrefixion+'<%=userid %>',oFAccount=imPrefixion+window.location.search.split("=")[1],oGroupid=imPrefixion+window.location.search.split("=")[1];
	clearUnread();
	var scrolled=false;
  ...
  zdy_ajax({
		...
		success: function(data,res){
			friendPhotos=data.headPortrait;
			zdy_ajax({
	  ...
	  function onMsg(msg) {
	      ...
	      if(msg.scene=="p2p" && msg.to==oMyaccounts  && msg.from==oFAccount){
	    	  var varId=yzm();
    		  if(msg.fromClientType=="Web"){
    			  if(msg.content){
    	    		  var urls=JSON.parse(msg.content)
    	    		  if(urls.type==2){
    	    			  $('#scroller').append('<div class="yourFriend" id="'+varId+'" ><img src="'+friendPhotos+'" class="_head" onclick="ckInfo1('+fAccount+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls.data.url+'"/></div>' + '</span></div><div  class="poles" style="height:2.5rem;"></div>'); 
    	    		  }else if(urls.type==3){
    	    			  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+friendPhotos+'" class="_head"  onclick="ckInfo1('+fAccount+')" /><span style="line-height:0.5rem;display:flex;">'+ '<img class="faudios" style="width:0.35rem;height:0.5rem;" FR="1" count="'+urls.data.times+'" onclick="fplayvoidce(this)" audio="'+urls.data.url+'" src="${path}/resource/img/images/FFFFFFFFFFFF.png"><span style="padding:0">'+urls.data.times+'\" '+'</span></span></div>');
    	    		  }
    	    		}else{
    	    			 $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+friendPhotos+'" class="_head"   onclick="ckInfo1('+fAccount+')" /><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",msg.text) + '</span></div>');
    	    		}
    		  }else{
    			  if( msg.type=="text"){
    	    		  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+friendPhotos+'" class="_head"      onclick="ckInfo1('+fAccount+')"/><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",msg.text) + '</span></div>');
    	    	  }else if(msg.type=="image"){
        			  $('#scroller').append('<div class="yourFriend" id="'+varId+'"><img src="'+friendPhotos+'" class="_head"      onclick="ckInfo1('+fAccount+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+msg.file.url+'"/></div>' + '</span></div><div  class="poles" style="height:2.5rem;"></div>'); 
				        	 ...
				        	 if(obj.msgs[i].from==oMyaccounts){
					    		  if(obj.msgs[i].type=="text" || obj.msgs[i].type=="custom"){
					    			  if(obj.msgs[i].content){
					    	    		  ...
					    	    		  $('#scroller').prepend('<div class="myself"><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",obj.msgs[i].text)+ '</span><img src="'+data.myInfo.avatar+'" class="_head" onclick="ckInfo1('+myaccounts+')"/></div>');
					    	    	  } 
				        		 ...
				        		 if(obj.msgs[i].type=="text" || obj.msgs[i].type=="custom"){
					    			  if(obj.msgs[i].content){
					    				  var urls=JSON.parse(obj.msgs[i].content);
					    	    		  if(urls.type==2){
					    	    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+friendPhotos+'" class="_head" onclick="ckInfo1('+fAccount+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+urls.data.url+'"/></div>' + '</span></div>'); 
					    	    		  }else{
					    	    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+friendPhotos+'" class="_head" onclick="ckInfo1('+fAccount+')"/><span style="line-height:0.5rem;display:flex;">'+ '<img class="faudios" style="width:0.35rem;height:0.5rem;" FR="1" count="'+urls.data.times+'" onclick="fplayvoidce(this)" audio="'+urls.data.url+'" src="${path}/resource/img/images/FFFFFFFFFFFF.png"><span style="padding:0">'+urls.data.times+'\" '+'</span></span></div>');
					    	    		  }
					    	    	  }else if(obj.msgs[i].type=="text" && !obj.msgs[i].content){
					    	    		  $('#scroller').prepend('<div class="yourFriend"><img src="'+friendPhotos+'" class="_head" onclick="ckInfo1('+fAccount+')"/><span>' + $.fn.decodeOoutEmoj("${path}/resource/img/emoji/",obj.msgs[i].text) + '</span></div>');
					    	    	  } 
					    		  }else if(obj.msgs[i].type=="image"){
					    			  $('#scroller').prepend('<div class="yourFriend" ><img src="'+friendPhotos+'" class="_head" onclick="ckInfo1('+fAccount+')" /><span>' + '<img class="messageIMGS" style="max-width:3.5rem;max-height:3.5rem;" onclick="showimgs(this)" src="'+obj.msgs[i].file.url+'"/></div>' + '</span></div>'); 
					    		  }else if(obj.msgs[i].type=="audio"){
