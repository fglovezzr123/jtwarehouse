...
public class SysAction extends BaseAction {
	...
	public String mlogin(HttpServletRequest request, String code, String state) throws UnsupportedEncodingException {
		...
		/*
		 * if (StringUtils.isEmpty(woid)) { if (!StringUtils.isEmpty(code)) { url =
		 * "https://api.weixin.qq.com/sns/oauth2/access_token?" + "appid=" + wx_appid +
		 * "&secret=" + wx_secret + "&code=" + code + "&grant_type=authorization_code";
		 * System.out.println("url：" + url); rs = HttpClientUtil.sendGetRequest(url,
		 * null); rsmap = JsonUtil.parseJSON2Map(rs); wxopenid = (String)
		 * rsmap.get("openid"); access_token = (String) rsmap.get("access_token"); } }
		 * else { wxopenid = woid; access_token = (String)
