...
--%>
	...
	<form  action="entryDescConfig/update.do"  id="updateQfyAbout_form"  method="post"   onsubmit="return validateSubmitForm(this)">
		...
		<div class="divider"></div>
		  <table class="table table-nobordered " style="margin-top: 25px;">
		 	  ...
		 	  <tr>
			    	...
			    	<td >
                		<span id="qfyAboutImgs" title="点击删除" ></span>
...
function save(){
	...
	$('.up_pic_img').each(function(){
		qfyAboutPicUrl = $(this).attr("srcpath");
	});
	if($.trim(qfyAboutPicUrl).length <= 0){
		Msg.alert("提示","请上传标签图片！","warning",null);
		return false;
	}else{
		$("#imgPath").val(qfyAboutPicUrl);
	}
	if(flag){
		if(!validateSubmitForm($('#updateQfyAbout_form'))){
			return;
		}
	}
}
...
//var qfyAboutPicUrl = $("#imgPath").val();
var qfyAboutPicUrl = '${dto.imgPath}';
if($.trim(qfyAboutPicUrl).length > 0){
	$("#qfyAboutImgs").html("");
	var imgHtml = "<img title='点击删除' onclick='delPic(this)'   srcpath='"+qfyAboutPicUrl+"' src='"+_oss_url+qfyAboutPicUrl+"' class='up_pic_img' style='width:375px; height:500px;'/>";
	$("#qfyAboutImgs").append(imgHtml);
}
...
function initUploadPicFrame1(){
	...
	frameParam.picRepository = "qfyabout";
...
function picUploadCallback(data){
	if (data.returnCode == "1"){
		var qfyAboutPicUrl = data.picPath;
		if(qfyAboutPicUrl.length > 0){
			var imgHtml = "<img title='点击删除' onclick='delPic(this)' srcpath='"+data.picPath+"' src='"+_oss_url+qfyAboutPicUrl+"' class='up_pic_img' style='width:375px; height:500px;'/>";
    		$("#qfyAboutImgs").append(imgHtml);
		}else{
			alert("上传失败,请稍后再试");
		}
