		...
		$('#<%=request.getParameter("rel")%>_datagrid').datagrid(
			{
				...
				columns : [ [
						...
						},
						{
							...
							align : "center",
							formatter : function(value, row, index) {
								return new Date(row.stime).format("yyyy-MM-dd HH:mm")+"至"
										+new Date(row.etime).format("yyyy-MM-dd HH:mm");
							}
						},
						{
								field : "sta",
						...
						
						{
								...
								formatter : function(rowIndex, rowData) {
									...
									if(status<5){
										if(sta==0){
											str="<a href = 'javaScript:void(0)' onclick = checkdelay('"+rowData.id+"')>编辑</a>";
										}else if(sta==1){
											str="已同意";
										}else if(sta==2){
											str="已拒绝";
