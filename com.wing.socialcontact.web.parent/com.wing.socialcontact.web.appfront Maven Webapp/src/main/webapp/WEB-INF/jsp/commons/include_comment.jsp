			...
			if(res.code == 0){
				...
				$.each(res.dataobj, function(i, n){
					...
					if(n.industry){
						...
						s += '				<b >'+n.subcount+'</b>';
						s += '			</span>';
						s += '			<span style="'+backgrounds+'" isThumbup='+n.isThumbup+' class="active_A givecomment" onclick="thumbup(this,\''+n.id+'\')"><b>'+n.count+'</b></span>';
						s += '		</div>';
						s += '	</div>';
						s += '	<div class="rm">';
...
function showimgs2(obj){	
 ...
 var comment_sub=function(_id){
    	self.location.href='${path}/m/comment/selCommentByPid.do?parentId='+_id;
    }
