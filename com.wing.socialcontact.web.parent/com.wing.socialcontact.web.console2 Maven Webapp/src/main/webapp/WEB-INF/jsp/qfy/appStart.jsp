...
--%>
	...
	<form  action="entryDescConfig/update.do"  id="updateAppStart_form"  method="post"   onsubmit="return validateSubmitForm(this)">
		...
		<div class="divider"></div>
		  <table class="table table-nobordered " style="margin-top: 25px;">
		 	  ...
		 	  <tr>
			    	...
			    	<td >
                		<span id="appStartImgs" title="点击删除" ></span>
                	   	<input  onclick="doSelectPicAppStart()"  type="button" value="上传图片" id="picturefile" class="form_shangchuan" />
	                    <iframe id="uploadPicFrame" src="" style="display:none;"></iframe>  
 	                    <strong style="color:#F00">尺寸375*750px。</strong>       
                	</td>
			  </tr>
			  <tr>
...
function save(){
	var appStartFlag = true;
	var appStartPicUrl = "";
	$('.up_pic_img').each(function(){
		appStartPicUrl = $(this).attr("srcpath");
	});
	if($.trim(appStartPicUrl).length <= 0){
		Msg.alert("提示","请上传标签图片！","warning",null);
		return false;
	}else{
		$("#imgPath").val(appStartPicUrl);
	}
	if(appStartFlag){
		if(!validateSubmitForm($('#updateAppStart_form'))){
			return;
		}
	}
}
...
var appStartPicUrl = '${dto.imgPath}';
if($.trim(appStartPicUrl).length > 0){
	$("#appStartImgs").html("");
	var imgHtml = "<img title='点击删除' onclick='delPic(this)'   srcpath='"+appStartPicUrl+"' src='"+_oss_url+appStartPicUrl+"' class='up_pic_img' style='width:375px; height:750px;'/>";
	$("#appStartImgs").append(imgHtml);
}
function doSelectPicAppStart() {
...
initUploadPicFrameAppStart();
function initUploadPicFrameAppStart(){
	...
	frameParam.picRepository = "appstart";
...
function picUploadCallback(data){
	if (data.returnCode == "1"){
		var appStartPicUrl = data.picPath;
		if(appStartPicUrl.length > 0){
			var imgHtml = "<img title='点击删除' onclick='delPic(this)' srcpath='"+data.picPath+"' src='"+_oss_url+appStartPicUrl+"' class='up_pic_img' style='width:375px; height:750px;'/>";
    		$("#appStartImgs").append(imgHtml);
		}else{
			alert("上传失败,请稍后再试");
		}
