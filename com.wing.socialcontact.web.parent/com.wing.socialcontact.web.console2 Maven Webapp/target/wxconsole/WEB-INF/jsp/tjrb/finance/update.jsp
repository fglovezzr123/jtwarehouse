...
--%>
	...
	<form  action="quickDetailBanner/update.do"  id="quickDetailBanner_form"  method="post"   onsubmit="return validateSubmitForm(this)">
			...
			<tr>
				...
				<td>
					<select name="financeType" id="financeType" required="true" style="width: 120px;">
						<option value="6" <c:if test="${financeProduct.financeType==6}">selected</c:if>>融资租赁</option>
						<option value="7" <c:if test="${financeProduct.financeType==7}">selected</c:if>>商业保理</option>
						<option value="8" <c:if test="${financeProduct.financeType==8}">selected</c:if>>典当抵押</option>
						<option value="9" <c:if test="${financeProduct.financeType==9}">selected</c:if>>抵押贷款</option>
					</select>
				</td>
		...
		<input type="hidden" name="id" value="${financeProduct.id}" />
